...
def main():
    ...
    model_folder = os.path.join('../models/LOF', policy_env_name)
    print(model_folder)

    model_file_pattern = os.path.join(model_folder, f"pipeline_lof_*.pkl")
    ...
    for i, model in enumerate(loaded_models):
        for j in range(args.n_exp_per_model):
            ...
            result[f"lof_{i}"][f"exp_{j}"] = {"scores_semantic":scores_semantic.tolist(),
    ...
    result_file = f"lof-{args.env}.json"
